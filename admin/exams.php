...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reopen_exam_id'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare('SELECT student_id FROM students WHERE student_id NOT IN (SELECT student_id FROM exam_attempts WHERE exam_id = ?)');
        $stmt->execute([$exam_id]);
        $students = $stmt->fetchAll(PDO::FETCH_COLUMN);

        // Insert into reopened_exams for each student
        $insertStmt = $pdo->prepare('INSERT INTO reopened_exams (exam_id, student_id) VALUES (?, ?)');
        foreach ($students as $student_id) {
            // Avoid duplicates
            $checkStmt = $pdo->prepare('SELECT COUNT(*) FROM reopened_exams WHERE exam_id = ? AND student_id = ?');
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        while ($exam = $stmt->fetch()) {
                            ...
                            echo "<form method='POST' action='exams.php' style='display:inline;'>
                                    <input type='hidden' name='reopen_exam_id' value='{$exam['exam_id']}'>
